    ...
    <div class="control-bar">
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          ...
          <!-- Verfuegbare Aktionen Tooltip -->
          <div class="actions-tooltip">
            <div class="tooltip-title">Verfuegbare Aktionen</div>
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div class="timeline-feed">
        ...
        <TransitionGroup name="timeline-item">
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                <!-- CREATE_POST: Beitrag veroeffentlichen -->
                ...
                <!-- CREATE_COMMENT: Kommentar abgeben -->
                ...
                <!-- Generischer Fallback: Unbekannter Typ oder Inhalt nicht oben verarbeitet -->
        ...
        <div v-if="allActions.length === 0" class="waiting-state">
          ...
          <span>Warte auf Agent-Aktionen...</span>
        </div>
      </div>
    </div>
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Von Step2 uebergebene maximale Rundenzahl
  minutesPerRound: {
    type: Number,
...
const phase = ref(0) // 0: Nicht gestartet, 1: Laeuft, 2: Abgeschlossen
const isStarting = ref(false)
const isStopping = ref(false)
const startError = ref(null)
const runStatus = ref({})
const allActions = ref([]) // Alle Aktionen (inkrementell akkumuliert)
const actionIds = ref(new Set()) // Aktions-ID-Menge fuer Deduplizierung
const scrollContainer = ref(null)

// Computed
// Aktionen chronologisch anzeigen (neueste unten)
const chronologicalActions = computed(() => {
  return allActions.value
})

// Aktionszaehlung pro Plattform
...
// Simulierte verstrichene Zeit formatieren (berechnet aus Runden und Minuten pro Runde)
...
// Simulierte verstrichene Zeit der Twitter-Plattform
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Simulierte verstrichene Zeit der Reddit-Plattform
...
// Alle Zustaende zuruecksetzen (fuer Neustart der Simulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
  ...
  stopPolling()  // Vorherige laufende Abfragen stoppen
}
...
const doStartSimulation = async () => {
  ...
  // Alle Zustaende zuruecksetzen, um Einfluss der vorherigen Simulation zu vermeiden
  resetAllState()
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(`Maximale Simulationsrunden festgelegt: ${props.maxRounds}`)
    }
    ...
    addLog(`✗ Startfehler: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`Stopp fehlgeschlagen: ${res.error || 'Unbekannter Fehler'}`)
    }
  } catch (err) {
    addLog(`Stoppfehler: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Rundenaenderungen jeder Plattform erkennen und protokollieren
      ...
      // Pruefen, ob Simulation abgeschlossen (ueber runner_status oder Plattform-Abschlussstatus)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'

      // Zusaetzliche Pruefung: Falls Backend runner_status noch nicht aktualisiert hat, Plattformen aber Abschluss melden
      const platformsCompleted = checkPlatformsCompleted(data)

      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Alle Plattformsimulationen als beendet erkannt')
        }
    ...
    console.warn('Laufstatus-Abruf fehlgeschlagen:', err)
  }
}

// Pruefen, ob alle aktivierten Plattformen abgeschlossen sind
const checkPlatformsCompleted = (data) => {
  // Wenn keine Plattformdaten vorhanden, false zurueckgeben
  if (!data) return false

  // Abschlussstatus der Plattformen pruefen
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // Wenn mindestens eine Plattform abgeschlossen ist, pruefen ob alle aktivierten Plattformen fertig sind
  // Plattformaktivierung ueber actions_count bestimmen (count > 0 oder running war true)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // Wenn keine Plattform aktiviert ist, false zurueckgeben
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Vollstaendige Aktionsliste ueber all_actions abrufen
      const serverActions = res.data.all_actions || []
      
      // Neue Aktionen inkrementell hinzufuegen (mit Deduplizierung)
      ...
      // Nicht automatisch scrollen, Benutzer kann Zeitachse frei durchsehen
      // Neue Aktionen werden unten angefuegt
    }
  } catch (err) {
    console.warn('Detailstatus-Abruf fehlgeschlagen:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Berichtsgenerierungsanfrage gesendet, bitte warten...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('Berichtsgenerierung wird gestartet...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Berichtsgenerierungsaufgabe gestartet: ${reportId}`)

      // Zur Berichtsseite weiterleiten
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ Berichtsgenerierung starten fehlgeschlagen: ${res.error || 'Unbekannter Fehler'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ Berichtsgenerierung Startfehler: ${err.message}`)
    isGeneratingReport.value = false
  }
}
