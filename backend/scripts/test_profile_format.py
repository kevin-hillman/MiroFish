"""
Testen, ob die Profilformatgenerierung den OASIS-Anforderungen entspricht
Validierung:
1. Twitter-Profil im CSV-Format generiert
2. Reddit-Profil im detaillierten JSON-Format generiert
"""
...
def test_profile_formats():
    """Profilformate testen"""
    print("=" * 60)
    print("OASIS-Profilformat-Test")
    print("=" * 60)
    
    # Test-Profildaten erstellen
    test_profiles = [
        OasisAgentProfile(
            user_id=0,
    ...
    # Temporaeres Verzeichnis verwenden
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print("\n1. Twitter-Profil testen (CSV-Format)")
        print("-" * 40)
        ...
        print(f"   Zeilenanzahl: {len(rows)}")
        print(f"   Tabellenkopf: {list(rows[0].keys())}")
        print(f"\n   Beispieldaten (1. Zeile):")
        ...
        print("\n2. Reddit-Profil testen (detailliertes JSON-Format)")
        print("-" * 40)
        ...
        print(f"   Eintragsanzahl: {len(reddit_data)}")
        print(f"   Felder: {list(reddit_data[0].keys())}")
        print(f"\n   Beispieldaten (1. Eintrag):")
        print(json.dumps(reddit_data[0], ensure_ascii=False, indent=4))
        
        # Felder des detaillierten Formats validieren
...
def show_expected_formats():
    """Erwartete OASIS-Formate anzeigen"""
    print("\n" + "=" * 60)
    print("OASIS erwartete Profilformat-Referenz")
    print("=" * 60)
...
1,user1,User One,Tech enthusiast and coffee lover.,200,250,1000,2023-01-02"""
    ...
    print("\n2. Reddit-Profil (detailliertes JSON-Format)")
    print("-" * 40)
    reddit_example = [
        {
